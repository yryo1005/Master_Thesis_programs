...
RESULT_DIR = f"../../results/GUMI_T_2/{USE_UNREAL_IMAGE}_{USE_WORD_IMAGE}_{USE_UNIQUE_NOUN_BOKE}_{MIN_STAR}_{MIN_APPER_WORD}_{MIN_SENTENCE_LENGTH}_{MAX_SENTENCE_LENGTH}_{EPOCH}_{BATCH_SIZE}_{LEARNING_RATE}_{FEATURE_DIM}_{NUM_HEADS}/"

if not os.path.exists("../../results/GUMI_T_2/"):
    os.mkdir("../../results/GUMI_T_2/")
...
class TransformerBokeGeneratorModel(nn.Module):
    def __init__(self, num_image_patch, image_feature_dim, num_word, sentence_length, feature_dim = 1024, num_heads = 4):
        ...
        self.cross_attention = MultiHeadCrossAttentionBlock(feature_dim = feature_dim, num_heads = num_heads)
    ...
    def forward(self, image_features, sentences):
        ...
        tgt = self.cross_attention( tgt = tgt, src = src, attn_mask = attn_mask )
        return self.fc2( tgt )
